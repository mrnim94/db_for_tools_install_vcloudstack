pipeline {
    ...
    environment {
        nameImage = "docker.nimtechnology.com/tool/db"
    ...
    stages {
        stage('Check GIT') {
            steps {
                checkout([$class: 'GitSCM', 
                    ...
                    gitTool: 'Default', 
                    submoduleCfg: [], 
                    userRemoteConfigs: [[credentialsId: 'Jenkin-login-Gitlab', url: 'https://gitlab.nimtechnology.com/nim/db_for_tools_install_vcloudstack.git']]
                ])
            }
        }
        ...
        stage('Prepare ENV for Build') {
            steps {
                script {
                    IMAGE_NAME = "docker.nimtechnology.com/tool/db"
